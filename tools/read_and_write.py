...
def load_data_path_part(shapenet_path):
    target_obj_file = 'model_normalized.obj'
    data_path = []
    for root, dirs, files in os.walk(shapenet_path, topdown=True):
        if target_obj_file in files:
            obj_path = os.path.join(root, target_obj_file)
            rendering_dir = root.replace('ShapeNetCore.v2_normalized', 'ShapeNetRenderings').replace('models', '')
            output_folder = '/'.join(rendering_dir.split('/')[:-2])
            # if os.path.exists(output_folder) and len(os.listdir(output_folder)) >= 50: #and len(os.listdir(rendering_dir)) == 40:
            #     print('exceed1')
            count = 0
            ...
            elif os.path.exists(rendering_dir) and len(os.listdir(rendering_dir)) == 80:
                print('exceed12')
            elif os.path.exists(rendering_dir) and len(os.listdir(rendering_dir)) < 40:
                print('do not consider')
            else:
                #print(obj_path)
                #print(len(os.listdir(output_folder)), len(os.listdir(rendering_dir)))
                data_path.append(obj_path)
    return data_path
