...
def render(viewpoint, viewpoint_id, rendering_dir):
    ...
    K, RT, camera_info = get_3x4_P_matrix_from_blender(cam_obj)
    ...
    return camera_info, camera_angle_x
...
def render_obj_by_vp_lists(rendering_dir, viewpoints):
    ...
    for vp_id, vp in enumerate(vp_lists):
        ...
        camera_info, camera_angle_x = render(vp, vp_id, rendering_dir)
        frame_info = {'file_path': os.path.join(rendering_dir, 'color_' + str(vp_id+1).zfill(3)),
                      'transform_matrix': camera_info['trm_matrix'], #'focal': camera_info['focal'],
                      'camera_angle_x': camera_angle_x}
        save_info['frames'].append(frame_info)

    with open(rendering_dir + 'transforms_new.json', 'w') as f:
...
def get_3x4_RT_matrix_from_blender(cam):
    ...
    trm_matrix = listify_matrix(cam.matrix_world)
    ...
    return RT, trm_matrix

def get_3x4_P_matrix_from_blender(cam):
    K, focal = get_calibration_matrix_K_from_blender(cam.data)
    RT, trm_matrix = get_3x4_RT_matrix_from_blender(cam)
    return K, RT, {'focal': focal, 'trm_matrix': trm_matrix}
...
if __name__ == '__main__':
    ...
    for model in result_list:
        ...
        count = 0
        for obj_file in glob.glob(output_folder + '/*'):
            if 'transforms.json' in glob.glob(obj_file + '/*'):
                count += 1

        if count >= 50:
            print('exceed render all!')
            continue

        # if len(os.listdir(output_folder)) >= 50:
        #     print ('exceed render all!')
        #     continue
        ...
        if len(os.listdir(rendering_dir)) == 80:
